...
country COUNTRIES.COUNTRY_ID%TYPE;
BEGIN
    SELECT COUNTRY_ID INTO country
    FROM COUNTRIES
    WHERE COUNTRY_NAME = country_name_input;


    SELECT COUNT(DEPARTMENTS.DEPARTMENT_ID), COUNT(EMPLOYEES.EMPLOYEE_ID) INTO departments_count, employees_count
    FROM DEPARTMENTS
    INNER JOIN LOCATIONS ON DEPARTMENTS.LOCATION_ID = LOCATIONS.LOCATION_ID
    RIGHT JOIN EMPLOYEES ON DEPARTMENTS.DEPARTMENT_ID = EMPLOYEES.DEPARTMENT_ID
    WHERE LOCATIONS.COUNTRY_ID = country;
